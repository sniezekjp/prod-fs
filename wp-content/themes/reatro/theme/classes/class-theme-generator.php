		...
		if(is_array($ids) && !empty($ids))
		{
			...
			foreach ($ids as $id) {
				...
				if(is_singular(array( 'post', 'portfolio' )) && $lightbox==false)
				{
					...
					$html .= get_the_post_thumbnail(array($width, $height)).'<div class="overlay"></div>';
				}
				else
				{
					...
					$html .= '<a href="'.$link.'" '.$class.$rel.$tilte.'>'.get_the_post_thumbnail(array($width, $height)).'<div class="overlay"></div></a>';
				}
